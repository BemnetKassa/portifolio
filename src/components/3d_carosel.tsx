...
const ThreeDCarousel = ({
  ...
  const [selectedProject, setSelectedProject] = useState<ThreeDCarouselItem | null>(null);
    ...
    <>
                      ...
                      <div className="mt-4">
                        <div className="flex flex-wrap gap-2 mb-4">
                          {item.tags.map((tag, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-1 bg-gray-50 text-blue-800 border-blue-800 rounded-full text-xs animate-pulse-slow"
                            >
                              {tag}
                            </span>
                          ))}
      ...
      {selectedProject && (
        <Dialog open={!!selectedProject} onOpenChange={() => setSelectedProject(null)}>
          <DialogContent className="bg-card text-foreground max-w-2xl w-full max-h-[90vh] flex flex-col">
            <DialogHeader>
              <DialogTitle className="text-3xl font-bold gradient-text">{selectedProject.title}</DialogTitle>
              <DialogDescription className="text-muted-foreground">{selectedProject.brand}</DialogDescription>
            </DialogHeader>
